          ...
          <div className="user-card card" key={u.id}>
            <div className="user-top">
              ...
              <div className="user-badges">
                ...
                {/** Normalize status for display; treat missing status as PENDING for review UX */}
                {(() => {
                  const displayStatus = (u.status || 'PENDING').toString();
                  const cls = `status-pill status-${displayStatus.toLowerCase()}`;
                  return <div className={cls}>{displayStatus}</div>;
                })()}
              </div>
            </div>
...
function AdminDashboard() {
  ...
  // expose helpers for quick console seeding during development
  useEffect(() => {
      ...
      <div className="admin-body">
        <aside className="admin-sidebar">
          <nav>
            ...
            <button className="seed-btn" style={{ marginTop: 10 }} onClick={seedSample}>Seed sample data</button>
          </nav>
        </aside>
